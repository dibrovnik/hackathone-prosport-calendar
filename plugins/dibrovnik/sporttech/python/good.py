...
async def handle_repeat_request(callback_query: CallbackQuery):
    ...
    if result is not None:
        ...
        if response.status_code==200:
            ...
            keyboard.button(text="Изменить номер телефона", callback_data="change_phone")
            if callback_query.message.text != "Произошла ошибка при получении информации о мероприятиях. Пожалуйста, попробуйте позже или проверьте введеный номер телефона.":
                await callback_query.message.edit_text("Произошла ошибка при получении информации о мероприятиях. Пожалуйста, попробуйте позже или проверьте введеный номер телефона.", reply_markup=keyboard.as_markup())
    else:
...
async def handle_contact(message: Message):
    ...
    if contact.user_id == user_id:
        ...
        await message.answer("Ваш номер телефона успешно сохранен!", reply_markup=types.ReplyKeyboardRemove())
        await send_event_info(user_id, message)

    else:
        keyboard = InlineKeyboardBuilder()
        keyboard.button(text="Назад", callback_data="repeat_request")
        await message.answer("Пожалуйста, отправьте ваш собственный номер телефона.", reply_markup=keyboard.as_markup())
